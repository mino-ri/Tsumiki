...
{
    public const int MaxBufferLength = 32768;
    ...
    public int DelaySampleCount = Math.Min((int)(unit.Delay * sampleRate / 1000), MaxBufferLength);
    ...
    public void Recalculate(double sampleRate)
    {
        ...
        DelaySampleCount = Math.Min((int)(_unit.Delay * sampleRate / 1000), MaxBufferLength);
...
[AudioTiming]
[method: InitTiming]
internal struct DelayChannel(DelayConfig config)
{
    const float FeedbackThreshold = 1f / 8192f;

    private readonly DelayConfig _config = config;
    private readonly float[] _buffer = GC.AllocateArray<float>(DelayConfig.MaxBufferLength, true);
...
[AudioTiming]
[method: InitTiming]
internal struct Delay(DelayConfig config)
{
    private readonly DelayConfig _config = config;
    private DelayChannel _left = new(config);
    private DelayChannel _right = new(config);
